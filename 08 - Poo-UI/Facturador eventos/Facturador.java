...
public class Facturador extends JFrame{
	...
	public void initComponent(){
		...
		*/
		...
		ActionListener event_click_vendedor = new ActionListener() {
			public void actionPerformed(ActionEvent event){
				buscarVendedor();
			}
		};
	...
	public void buscarCliente(){
		...
		// Recorrer el arreglo y compara la cedula
		for (int i = 0; i < listaUsuarios.length; i++) {
			if(listaUsuarios[i] != null && listaUsuarios[i].getRol().equals("CLIENTE") && listaUsuarios[i].getCedula().equals(cedula)){
				...
				input_direccion_cliente.setText(listaUsuarios[i].getDureccion());
